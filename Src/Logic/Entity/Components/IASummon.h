#ifndef __Logic_IASummon_H
#define __Logic_IASummon_H
...
namespace Logic 
{
	...
	class CIASummon : public IComponent
	{
		DEC_FACTORY(CIASummon);

	public:

		CIASummon() : IComponent() {}
	...
	protected:
		...
		CEntity *_target;
		std::vector<InfoMinion> _vecMinions;
		std::string _typeMinion;
		int _leaderLevel;
		int _leaderTier;
		float _distMinion;
		float _velRotationMinion;
		float _minDist;
		bool _canSendMinion;
		bool _canIA;
		int _delayCanInvoke;
		int _maxDelayCanInvoke;
		int _delaySendMinion;
		int _maxDelaySendMinion;
		int _maxMinions;
		

	}; // class Controller

	REG_FACTORY(CIASummon);
