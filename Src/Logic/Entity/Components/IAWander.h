#ifndef __Logic_IAWander_H
#define __Logic_IAWander_H

#include "Logic/Entity/Component.h"

namespace Logic 
{
	class CIAWander : public IComponent
	{
		DEC_FACTORY(CIAWander);

	public:

		CIAWander() : IComponent() {}
	...
	protected:
		...
		int _timeWait;
		int _timeWander;
		int _maxTimeWaitBase;
		int _maxTimeWaitNow;
		int _maxTimeWanderBase;
		int _maxTimeWanderNow;
		int _delaySideCollision;
		bool _isWaiting;
		bool _dirStablished;
		float _maxVelWander;
		bool _isTouchingSideTile;
		Vector3 _velMovement;
		Vector3 _direction;
		
	}; // class Controller

	REG_FACTORY(CIAWander);
