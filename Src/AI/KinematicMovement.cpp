...
namespace AI 
{
	...
	*/
	...
	void CKinematicStop::move(unsigned int msecs, AI::IMovement::DynamicMovement &currentProperties)
	{
		assert(_entity);

		currentProperties.linearSpeed *= currentProperties.slowdownFactor;

		if (currentProperties.linearSpeed.positionEquals(Vector3::ZERO, currentProperties.tolerance))
			currentProperties.linearSpeed = 0;
	}
