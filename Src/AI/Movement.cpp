...
namespace AI {
	...
	movimiento.
	*/
	const float IMovement::ATTENUATION = 1000.0f; // 1 segundo de frenada. Originalmente 250.0f
	/**
	...
	*/
	const float IMovement::SLOW_RADIUS = 200.0f; // 200px de radio. Originalmente 30.0f
	/////////////////////////////////////////
	/**
	...
	IMovement* IMovement::getMovement(int type, float maxLinearSpeed, float maxAngularSpeed, float maxLinearAccel, float maxAngularAccel)
	{
			switch (type) {
				...
				case MOVEMENT_KINEMATIC_FLEE:
					...
					break;
				case MOVEMENT_KINEMATIC_STOP:
					return  new CKinematicStop(maxLinearSpeed, maxAngularSpeed, maxLinearAccel, maxAngularAccel);
					break;
