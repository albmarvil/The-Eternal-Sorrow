...
-------------------------------TABLA EnemApprMDSummon---------------------------
EnemApprMDSummon = {
	--variables del enemigo

	id = 1,
	tier = 1,
	Archetype = "EnemApprMDSummon",
	...
	GetNextID = function (  )
		local id = EnemApprMDSummon.id + 1
		EnemApprMDSummon.id = id
		return id
	end,
}
...
EnemApprMDSummonContenedor = { 
	...
	CrecimientoDinero = function (level)
		...
		min = 2 * level + EnemApprMDSummonContenedor.rangoDineroBase.min
		max = 2 * level + EnemApprMDSummonContenedor.rangoDineroBase.max
...
Drop.Contenedores[EnemApprMDSummon.Archetype] = EnemApprMDSummonContenedor

-------------------------APUNTADO EN LA TABLA-----------------------

if SpawnerMgr.tiers["Tier"..EnemApprMDSummon.tier] == nil then
	SpawnerMgr.tiers["Tier"..EnemApprMDSummon.tier] = {Enemigos = {},}
end

table.insert(SpawnerMgr.tiers["Tier"..EnemApprMDSummon.tier].Enemigos, EnemApprMDSummon)